...
def _1_5():
    return lesson(
        "1.5", "Списки и операции над ними", "space", [
            ...
            visual(
                "Список как индексированный массив ячеек: показаны операции append (в конец), insert (в середину) и pop (удаление по индексу).",
                r'''   Индекс:  [0]      [1]       [2]      [3]      [4]
            ┌───────┬────────┬─────────┬────────┬────────┐
   missions │Apollo │Soyuz   │Vostok 1│Gemini  │Mercury │
            │  11   │  T-12  │        │   7    │   3    │
            └───────┴────────┴─────────┴────────┴────────┘
                ▲                ▲                ▲
                │                │                │
            missions[0]    insert(2,'V')    append('X')
            'Apollo 11'     сдвигает вправо   расширяет

   Операции:  lst[i]    — доступ по индексу
              lst[i]=x  — присвоение
              len(lst)  — длина
              lst[a:b]  — срез [a, b)'''
            ),
...
def _1_6():
    return lesson(
        "1.6", "Словари", "space", [
            ...
            visual(
                "Словарь как отображение ключ→значение: показаны три способа доступа (d[k], d.get(k), d.items()).",
                r'''  mission_data = {
      'mission'   : 'Apollo 11',
      'year'      : 1969,
      'duration'  : '8 days',
...
def _1_7():
    return lesson(
        "1.7", "Множества и кортежи", "space", [
            ...
            visual(
                "Множество как круг уникальных элементов (без порядка) и кортеж как неизменяемая строка данных фиксированной длины.",
                r'''   SET (уникальные, без порядка):         TUPLE (фиксированная строка):
   ╭─────────────────╮
   │  ✦  Mars        │             coords = (lon, lat, alt)
   │   ✦  Venus      │             ┌──────┬──────┬──────┐
   │     ✦  Jupiter  │             │ 12.5 │ 45.3 │  408 │
   │       (нет дублей)             └──────┴──────┴──────┘
   ╰─────────────────╯                  │      │      │
                                       x=lon  y=lat  z=alt
   Операции:  a | b  объединение      x, y, z = coords
              a & b  пересечение      (распаковка)
              a - b  разность
              x in a проверка         coords[0] = 99  ✗ TypeError!
                                        (immutable)'''
            ),
            example(
...
def _1_8():
    return lesson(
        "1.8", "Обработка строк", "space", [
            ...
            visual(
                "Строка как массив символов с индексацией: показаны позитивные/негативные индексы, срезы и примеры вызовов методов.",
                r'''   s = "Apollo 11"
       │  A  p  o  l  l  o  _  1  1  │
       │                              │
   idx: 0  1  2  3  4  5  6  7  8    (длина 9)
        ... -3 -2 -1   (негативные с конца)

   ┌────────────────────────────────────────────┐
   │  s[0]      → 'A'         s[-1]     → '1' │
   │  s[2:5]    → 'oll'       s[::-1]   → reverse│
   │  s.lower() → 'apollo 11'  s.upper() → 'APOLLO 11'│
   │  s.split() → ['Apollo','11']                │
   │  s.find('o')  → 1        s.replace('11','X') │
   │  ' '.join(['A','B']) → 'A B'                │
   └────────────────────────────────────────────┘

   ✦ Строки IMMUTABLE: s[0] = 'X'  → TypeError
   ✦ Нужно: s = s.replace(...)'''
            ),
            example(
