...
def _1_9():
    return lesson(
        "1.9", "Чтение и запись файлов", "space", [
            ...
            visual(
                "Жизненный цикл файла: диск → open() с режимом r/w/a → чтение/запись → close() (или автоматически через with).",
                r'''     📁 Файл на диске
     ┌──────────────────┐
     │ missions.txt     │
     │  ─────────────   │
     ...
     │  Vostok 1   \n   │
     └────────┬─────────┘
              │ open('missions.txt', mode, encoding='utf-8')
              ▼
     ┌────────────────────────────┐
     │  f = open(...)             │
     │  ┌─────┬─────┬──────┐      │
     ...
     │  ['Apollo 11', 'Apollo 12']│
     └────────┬───────────────────┘
              │ with ... as f:  (auto-close)
              ▼
     ┌────────────────────────────┐
     │  f.close()  ✓              │
     │  flush + release handle    │
     └────────────────────────────┘'''
            ),
            example(
...
def _1_10():
    return lesson(
        "1.10", "Работа с CSV", "space", [
            ...
            visual(
                "CSV-файл как текстовая таблица: строка заголовка с именами столбцов, далее строки данных, разделитель — запятая.",
                r'''   Файл launches.csv на диске (текст):
   ┌──────────────────────────────────────────────────┐
   │ mission,year,rocket,success,cost_mln              │  ← заголовок (header)
   │ Apollo 11,1969,Saturn V,1,250                    │
   │ Apollo 13,1970,Saturn V,0,375                    │  ← данные (rows)
   │ Artemis 1,2022,SLS,1,4000                        │
   └──────────────────────────────────────────────────┘
            │                │                │
            ▼                ▼                ▼
   ┌──────────────┐  ┌──────────────┐  ┌──────────────┐
   │  DictReader  │  │  reader      │  │  DictWriter  │
   │  (по имени)  │  │  (по индексу)│  │  (запись)    │
   └──────┬───────┘  └──────┬───────┘  └──────┬───────┘
          ▼                 ▼                 ▼
   {'mission':'Apollo 11',  [row[0],         writeheader()
    'year': 1969,           row[1], ...]     writerow({...})
    'rocket':'Saturn V',                     writerows([...])
    'success': 1,
    'cost_mln': '250'}
   ↑ значения приходят как STR (нужен int/float)'''
            ),
            example(
...
def _1_11():
    return lesson(
        "1.11", "Обработка ошибок", "space", [
            ...
            visual(
                "Поток выполнения try/except/else/finally: при исключении — переход в except, finally выполняется ВСЕГДА, else — только если ошибки не было.",
                r'''              ┌────────────────┐
              │  Начало        │
              └───────┬────────┘
                      ▼
    ...
    ┌────────────────┐   ┌────────────────┐
    │  except E:     │   │  else:         │
    │  handler()     │   │  success_path()│
    └───────┬────────┘   └───────┬────────┘
            └──────────┬─────────┘
                       ▼
              ┌────────────────┐
              │   finally:     │  ← «выполняется ВСЕГДА»
              │   cleanup()     │     (даже при return/raise)
              └───────┬────────┘
                      ▼
              ┌────────────────┐
              │   Продолжение  │
...
def _1_12():
    return lesson(
        "1.12", "Мини-проект: Анализ телеметрии ракеты", "space", [
            ...
            visual(
                "Конвейер мини-проекта по анализу телеметрии: CSV → фильтрация аномалий → агрегаты → отчёт и график.",
                r'''   ┌────────────────────┐
   │  telemetry.csv     │  читаем csv.DictReader
   │  time,alt,speed,   │
   │  temp,pressure     │
   └─────────┬──────────┘
             │  rows: List[Dict]
             ▼
   ┌────────────────────┐
   │  1. Фильтрация     │  [r for r in rows if r['temp']>3500]
   │     аномалий       │  → anomalies
   └─────────┬──────────┘
             │  filtered
             ▼
   ┌────────────────────┐
   │  2. Агрегация      │  max(alt), max(speed),
   │     статистик      │  avg(temp), count(anomalies)
   └─────────┬──────────┘
             │  stats dict
             ▼
   ┌────────────────────┐
   │  3. Формирование   │  {'max_alt': ..., 'anomalies': n}
   │     отчёта (dict)  │
   └─────────┬──────────┘
             │
             ▼
   ┌────────────────────┐
   │  4. Вывод / plot   │  print() / matplotlib
   └────────────────────┘'''
            ),
            example(
