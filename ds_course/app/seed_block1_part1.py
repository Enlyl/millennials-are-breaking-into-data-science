...
def _1_2():
    return lesson(
        "1.2", "Условия и ветвления", "space", [
            ...
            visual(
                "Дерево решений if/elif/else для определения статуса ракеты по высоте: проверяется порог в 100 км (линия Кармана).",
                r'''                          altitude_km
                              │
                              ▼
                  ┌───────────────────────┐
                  ...
                  └───────────┬───────────┘
                       ...
                       ▼       ▼
              ┌──────────┐  ┌────────────────────────┐
              │ "на земле"│  │ altitude_km < 1000 ?  │
              └──────────┘  └────────┬───────────────┘
                            │ Да    │   Нет
                            ▼       ▼
                    ┌──────────────┐  ┌──────────────┐
                    │"в атмосфере" │  │ "в космосе"  │
                    └──────────────┘  └──────────────┘
                                          │
                                          ▼
                                   🚀 ОРБИТА'''
...
def _1_3():
    return lesson(
        "1.3", "Циклы и итерации", "space", [
            ...
            visual(
                "Цикл for, перебирающий список планет: на каждой итерации счётчик i увеличивается, берётся следующий элемент списка.",
                r'''         ┌─────────────────────┐
         │  planets = [...]    │  ◀── список
         └──────────┬──────────┘
                    ...
                    ▼
    ┌───────────────────────────────────────────┐
    │  i = 0   │  planet = "Mercury"  │ print() │
    ├──────────┼──────────────────────┼─────────┤
    │  i = 1   │  planet = "Venus"    │ print() │
    ├──────────┼──────────────────────┼─────────┤
    │  i = 2   │  planet = "Earth"    │ print() │
    ├──────────┼──────────────────────┼─────────┤
    │  i = 3   │  planet = "Mars"     │ print() │
    └──────────┴──────────────────────┴─────────┘
                    │
                    ▼
            ┌──────────────┐
...
def _1_4():
    return lesson(
        "1.4", "Функции", "space", [
            ...
            visual(
                "Анатомия функции: вход → тело → выход",
                r"""
       вызывающий код                вызывающий код
            │                              │
            ▼                              ▼
   ┌────────────────┐             ┌────────────────┐
   │  f(x, y=10)    │  ────▶      │  f(x, y=10)    │
   │  def f(x, y):  │  вызов      │  def f(x, y):  │
   │    z = x + y   │             │    z = x + y   │
   │    return z    │             │    return z    │
   └────────────────┘             └────────────────┘
            │                              │
            ▼                              ▼
       результат None              результат = x+y

   Поток данных:
   ┌────────┐    ┌─────────┐    ┌────────┐
   │  args  │──▶│  тело   │──▶│ return │──▶ результат
   │ (x, y) │   │ (код)   │    │   z    │
   └────────┘    └─────────┘    └────────┘
"""
            ),
