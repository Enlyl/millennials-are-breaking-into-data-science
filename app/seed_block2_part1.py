...
def _2_1():
    return lesson(
        "2.1", "SELECT, FROM, WHERE", "gaming", [
            ...
            visual(
                "Поток SQL-запроса SELECT: из таблицы выбираются указанные столбцы, затем WHERE фильтрует строки.",
                r'''   ┌───────────────────────────────┐
   │  players (вся таблица)       │
   │  id │ name       │ score     │
   │  ───┼────────────┼───────────│
   ...
   └───────────────┬───────────────┘
                   │
   SELECT name, score          ← какие столбцы
                   │
                   ▼
   ┌───────────────────────────────┐
   │  name       │ score           │
...
def _2_2():
    return lesson(
        "2.2", "ORDER BY, LIMIT, DISTINCT", "gaming", [
            ...
            visual(
                "Конвейер ORDER BY → DISTINCT → LIMIT: неотсортированная таблица сортируется, дубли убираются, остаётся топ-N.",
                r'''   Исходная таблица (не отсортирована):
   ┌────────────────────┐
   │ name       │ score │
   │ ──────────┼───────│
   │ Bob        │  800  │
   ...
   └────────┬───────────┘
            │
   ORDER BY score DESC   ← сортировка по убыванию
            │
            ▼
   ┌────────────────────┐
   │ Diana      │ 2200  │ ─┐
   │ Alice      │ 1500  │  │
   │ Alice      │ 1500  │  │  LIMIT 3
   │ Bob        │  800  │  │   (или DISTINCT)
   │ Charlie    │  600  │ ─┘
   └────────┬───────────┘
            │
            ▼
   ┌────────────────────┐
   │ Diana      │ 2200  │  ← топ-3 результат
   │ Alice      │ 1500  │
   │ Alice      │ 1500  │
   └────────────────────┘'''
            ),
...
def _2_3():
    return lesson(
        "2.3", "Агрегатные функции: COUNT, SUM, AVG, MIN, MAX", "gaming", [
            ...
            visual(
                "Агрегатные функции «схлопывают» много строк в одно значение: COUNT, SUM, AVG, MIN, MAX.",
                r'''   ┌─────────────────────────────┐
   │  players                   │     ┌──────────────────────────┐
   │  ───────────────────────── │     │  Итог (одна строка)      │
   │  name    │ score           │     │  ─────────────────────── │
   │  Alice   │ 1500   ─────┐   │     │  COUNT(*) : 4            │
   │  Bob     │  800   ─────┤   │     │  SUM(score): 5100        │
   │  Charlie │  600   ─────┼───│────▶│  AVG(score): 1275.0      │
   │  Diana   │ 2200   ─────┘   │     │  MIN(score):  600        │
   │  ──────────── 4 строки ──  │     │  MAX(score): 2200        │
   └─────────────────────────┘     └──────────────────────────┘

   Много строк  ──── агрегаты ───▶  Одно значение
                    (сжатие)

   COUNT(*) считает все строки (включая NULL)
   COUNT(col) игнорирует NULL в col
   SUM/AVG/MIN/MAX — только по NOT NULL значениям'''
            ),
            example(
...
def _2_4():
    return lesson(
        "2.4", "GROUP BY и HAVING", "gaming", [
            ...
            visual(
                "GROUP BY разбивает строки по значению столбца на группы, для каждой считается агрегат. HAVING фильтрует готовые группы.",
                r'''   До GROUP BY (все строки вперемешку):       После GROUP BY team:
   ┌─────────────────────────┐                  ┌────────────────────────────────┐
   │ name    │ team   │score │                  │ team   │ COUNT(*)│ AVG(score)│
   │ ────────┼────────┼──────│                  │ ───────┼─────────┼───────────│
   │ Alice   │  red   │ 1500 │─┐                │  red   │    2    │  1150     │
   │ Bob     │  red   │  800 │─┤──▶ group red    │  blue  │    2    │  1450     │
   │ Charlie │  blue  │  600 │─┤                └────────────────────────────────┘
   │ Diana   │  blue  │ 2200 │─┘──▶ group blue       ▲
   └─────────────────────────┘                      │
...
def _2_5():
    return lesson(
        "2.5", "CASE WHEN", "gaming", [
            ...
            visual(
                "CASE WHEN как ветвление внутри SELECT: проверяется условие за условием, выбирается первое сработавшее, END обязателен.",
                r'''   score каждого игрока            CASE WHEN
   ┌────────────────┐              ┌──────────────────────────────────────┐
   │ name    │ score│              │ WHEN score >= 1500 THEN 'Pro'       │
   │ ────────┼──────│              │ WHEN score >= 1000 THEN 'Average'   │
   │ Alice   │ 1500 │──┐           │ WHEN score >=  500 THEN 'Rookie'    │
   │ Bob     │  800 │──┤    ────▶  │ ELSE                  'Newbie'      │
   │ Charlie │  600 │──┤           │ END AS rank                          │
   │ Diana   │  300 │──┘           └──────────────────────────────────────┘
   └────────────────┘                           │
                                               ▼
                                ┌────────────────────────┐
                                ...
                                │ Alice   │ Pro          │ ← 1500>=1500
                                │ Bob     │ Rookie       │ ← 800<1000
                                │ Charlie │ Rookie       │ ← 600<1000
                                │ Diana   │ Newbie       │ ← ELSE
                                └────────────────────────┘

   Порядок проверки: сверху вниз, первое совпадение → результат
   END обязателен!'''
            ),
            example(
...
def _2_6():
    return lesson(
        "2.6", "INNER JOIN, LEFT JOIN", "gaming", [
            ...
            visual(
                "INNER JOIN (диаграмма Венна, пересечение) и LEFT JOIN (все из левой + совпадения из правой или NULL).",
                r'''   players                teams
   ┌───────────────┐      ┌─────────────────┐
   │ id │ name     │      │ id │ team_name  │
   │ 1  │ Alice    │      │ 1  │ Red        │
   │ 2  │ Bob      │      │ 2  │ Blue       │
   │ 3  │ Charlie  │      │ 3  │ Green (no  │
   │ 4  │ Diana    │      │     │ players)   │
   └───────┬───────┘      └────────┬────────┘
           │                       │
           └────── ON p.team_id = t.id
   ...
   │  │     │╳     │ │              │  │  ███████  │    │
   │  ╰─────╯ ╰─────╯ │              │  ╰─────╯ ╰─────╯    │
   │  только общие    │              │  все из LEFT + NULL │
   │  (3 строки)     │              │  для несовпавших     │
   └──────────────────┘              │  (4 строки)          │
                                     └─────────────────────┘'''
            ),
            example(
...
def _2_7():
    return lesson(
        "2.7", "RIGHT JOIN, FULL JOIN, CROSS JOIN", "gaming", [
            ...
            visual(
                "Четыре типа JOIN через диаграммы Венна: RIGHT, FULL OUTER и декартово произведение CROSS JOIN.",
                r'''   RIGHT JOIN          FULL OUTER JOIN         CROSS JOIN (декартово)
   ╭─────╮ ╭─────╮      ╭─────╮ ╭─────╮         A={1,2}  B={x,y}
   │  p  │ │  t  │      │  p  │ │  t  │         ┌────┬────┐
   │     │█│█    │      │ ███████████ │         │ 1  │ x  │
   │     │█│█    │      │ ███████████ │         │ 1  │ y  │
   ╰─────╯ ╰─────╯      ╰─────╯ ╰─────╯         │ 2  │ x  │
   все из правой   все из обеих таблиц  │ 2  │ y  │
   + LEFT часть    (объединение)       └────┴────┘
   (3 строки)      (4 строки)          |A|*|B| строк

   ┌──────────────────┬────────────────────┬──────────────────────┐
   │ Тип JOIN         │ Возвращает         │ Размер результата    │
   ├──────────────────┼────────────────────┼──────────────────────┤
   │ RIGHT JOIN       │ все из правой      │ ≤ max(|L|,|R|)       │
   │ FULL OUTER JOIN  │ все из обеих       │ |L| + |R| − inner    │
   │ CROSS JOIN       │ каждая с каждой    │ |L| * |R|  ⚠ взрыв! │
   └──────────────────┴────────────────────┴──────────────────────┘'''
            ),
            example(
